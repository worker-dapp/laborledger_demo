...
const MyJobDetails = () => {
    ...
    useEffect(() => {
        if (location && location.latitude && location.longitude) {
            if (punchInDetails.length === 0) {  // Ensure we add punch-in only when location is available
                const punchInTime = new Date().toLocaleTimeString();
                setPunchInDetails((prevState) => [
                    ...prevState,
                    {
                    ...
                    <table className="min-w-full table-auto border-collapse border border-gray-300">
                        ...
                        <tbody>
                            {punchInDetails.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan="4" className="border px-4 py-2 text-center">No Punch-In Data</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
